...
const MenuCard = ({ id, title, price, shortDescription, longDescription, img, vegNonVeg, onClick }) => {
  const handleAddToCart = (id) => {
    alert(id);
  };

  const vegNonVegClass = vegNonVeg === "veg" ? "bg-green-500" : vegNonVeg === "non-veg" ? "bg-red-500" : "bg-gray-500";

  return (
    <Card className="flex items-center w-full relative overflow-hidden">
      <div className={`size-2 rounded-full absolute top-2 right-2 ${vegNonVegClass}`} title={vegNonVeg}></div>
